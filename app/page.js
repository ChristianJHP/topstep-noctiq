        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-8">
            {[
              ...
              {
                title: 'TradeZella',
                desc: 'The trade journal I use to track performance, review my edge, and catch patterns I\'d otherwise miss. Highly recommend if you\'re not already journaling every trade.',
                tag: 'Tool',
              ...
              <FadeIn key={i} delay={i * 60}>
                {item.href ? (
                  <a href={item.href} target="_blank" rel="noopener noreferrer" className="group block h-full p-6 rounded-xl border border-white/[0.06] bg-white/[0.01] hover:bg-white/[0.03] hover:border-white/[0.12] transition-all duration-300">
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border ${item.tagColor}`}>{item.tag}</span>
                    </div>
                    <h3 className="text-white font-semibold mb-2">{item.title}</h3>
                    <p className="text-sm text-neutral-500 leading-relaxed">{item.desc}</p>
                  </a>
