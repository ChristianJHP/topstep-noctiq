...
function CandlestickBackground() {
  ...
  useEffect(() => {
    ...
    const BASE_Y = canvas.height * 0.62
    const AMPLITUDE = 70

    // Generate realistic-ish random walk candles
    function generateCandles(count) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const wickUp = Math.random() * 4
        const wickDown = Math.random() * 4
    ...
    let candles = generateCandles(CANDLE_COUNT + 10)
    let offset = 0       // fractional scroll offset
    let formProgress = 0 // 0-1 progress on the forming (last) candle

    const SCROLL_SPEED = 0.004  // candles/frame – very slow drift
    const FORM_SPEED = 0.006    // how fast each new candle "prints"

    function priceToY(price, minP, range) {
      return BASE_Y - ((price - minP) / range) * AMPLITUDE
    }

    function drawFrame() {
      ctx.clearRect(0, 0, canvas.width, canvas.height)

      // slowly drift right-to-left
      offset += SCROLL_SPEED
      formProgress += FORM_SPEED

      if (formProgress >= 1) {
        formProgress = 0
        offset = Math.floor(offset) // snap
        candles.push(...generateCandles(1))
        if (candles.length > CANDLE_COUNT + 20) candles = candles.slice(10)
      }

      const startIdx = Math.floor(offset)
      const fracOffset = offset - startIdx
      const visible = candles.slice(startIdx, startIdx + CANDLE_COUNT + 2)
      ...
      visible.forEach((c, i) => {
        ...
        const progress = isForming ? formProgress : 1

        const x = startX + (i - fracOffset) * STEP
        if (x < -STEP || x > canvas.width + STEP) return

        const openY = priceToY(c.open, minP, range)
        const closeY = priceToY(c.close, minP, range)
        const highY = priceToY(c.high, minP, range)
        const lowY = priceToY(c.low, minP, range)

        const bullish = c.close >= c.open
        const bodyColor = bullish
          ? 'rgba(52,211,153,0.18)'   // emerald tint
        ...
        const bodyH = Math.max(1, Math.abs(closeY - openY)) * progress
        ...
        ctx.lineTo(x + CANDLE_W / 2, lowY * progress + bodyTop * (1 - progress))
        ctx.stroke()
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <FadeIn>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <a
                  href="https://discord.gg/n5etyMfq"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#5865F2] hover:bg-[#4752C4] text-white font-semibold text-sm transition-colors"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
          ...
          <FadeIn>
            <div className="rounded-2xl border border-white/[0.06] bg-gradient-to-br from-indigo-500/[0.06] via-purple-500/[0.03] to-transparent p-10 sm:p-14 text-center">
              ...
              <p className="text-neutral-400 max-w-md mx-auto mb-8">
                I post new content regularly on TikTok covering algo trading, backtesting, prop firms, and the math behind it all. Join the Discord to connect with the community.
              </p>
              <div className="flex flex-wrap items-center justify-center gap-4">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-[#5865F2] hover:bg-[#4752C4] text-white font-semibold text-sm transition-colors"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.002.022.015.043.03.056a19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
                  </svg>
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-neutral-500 hover:text-[#5865F2] transition-colors"
              aria-label="Discord"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.002.022.015.043.03.056a19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
