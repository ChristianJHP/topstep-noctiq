 ...
 * @param {number} options.tpQuantity - Contracts for take profit (default: quantity, for runner strategy)
 */
async function placeBracketOrder(action, stopPrice, takeProfitPrice, quantity = 1, options = {}) {
  ...
  console.log(`[ProjectX] Quantities - Entry: ${quantity}, SL: ${quantity}, TP: ${tpQuantity}${runnerQuantity > 0 ? `, Runner: ${runnerQuantity}` : ''}`);
  console.log(`[ProjectX] skipCleanup: ${skipCleanup}`);
  ...
  try {
    ...
    // 3. Place take profit (opposite side) - may be partial for runner strategy
    const tpSide = action.toLowerCase() === 'buy' ? 'sell' : 'buy';
    console.log(`[ProjectX] Placing take profit at ${takeProfitPrice} for ${tpQuantity} contract(s)...`);
    try {
      tpOrder = await placeLimitOrder(tpSide, takeProfitPrice, tpQuantity);
      if (runnerQuantity > 0) {
        console.log(`[ProjectX] Runner strategy active: ${runnerQuantity} contract(s) will run until next signal`);
      }
    } catch (tpError) {
