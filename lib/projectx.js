...
async function placeBracketOrder(action, stopPrice, takeProfitPrice, quantity = 1) {
  ...
  try {
    // 0. Clean up: Close any existing positions and cancel all open orders
    console.log('[ProjectX] Cleaning up existing positions and orders...');

    // Close existing positions
    const positions = await getPositions();
    if (positions && positions.length > 0) {
      console.log(`[ProjectX] Found ${positions.length} existing position(s), closing...`);
    ...
    console.log('[ProjectX] Cleanup complete, placing new bracket order...');
...
async function getPositions() {
  ...
  console.log('[ProjectX] Fetching positions...');
  ...
  const data = await response.json();
  console.log('[ProjectX] Positions:', JSON.stringify(data));
...
async function getOpenOrders() {
  ...
  console.log('[ProjectX] Fetching open orders...');
  ...
  const response = await retryWithBackoff(async () => {
    return await fetchWithTimeout(`${BASE_URL}/Order/search`, {
      ...
      body: JSON.stringify({
        accountId: accountId,
        // Only get working/pending orders
      }),
    });
  }, 'GetOpenOrders');
  ...
  const data = await response.json();
  console.log('[ProjectX] Open orders:', JSON.stringify(data));
