...
async function getAccountId() {
  ...
  const data = await response.json();
  console.log('[ProjectX] Account search response:', JSON.stringify(data));
  ...
  // Find matching account if configured
  const configuredAccountId = process.env.PROJECTX_ACCOUNT_ID;
  let account = accounts[0];

  if (configuredAccountId) {
    // Try to find the matching account by name or any field containing the ID
    const matchingAccount = accounts.find(a =>
      a.name === configuredAccountId ||
      a.accountName === configuredAccountId ||
      String(a.id) === configuredAccountId ||
      JSON.stringify(a).includes(configuredAccountId)
    );

    if (matchingAccount) {
      account = matchingAccount;
      console.log(`[ProjectX] Found matching account for ${configuredAccountId}`);
    } else {
      console.log(`[ProjectX] Using first account (configured ID not found in accounts)`);
    }
  }
...
async function getAccountDetails() {
  ...
  const response = await fetchWithTimeout(`${BASE_URL}/Account/${accountId}`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
