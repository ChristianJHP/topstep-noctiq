 ...
 *
 * EDGE CASES HANDLED:
 * - RTH boundary (stops 30 seconds before close)
...
const RISK_LIMITS = {
  ...
  RTH_END_MINUTE: 0,
  RTH_END_BUFFER_SECONDS: 30, // Stop trading 30 seconds before close
};
 ...
 * Includes 30-second buffer before close to prevent edge cases
 */
function isWithinRTH() {
  const { hour, minute, second } = getETTimeComponents();

  const currentSeconds = hour * 3600 + minute * 60 + second;
  const startSeconds = RISK_LIMITS.RTH_START_HOUR * 3600 + RISK_LIMITS.RTH_START_MINUTE * 60;
  const endSeconds = RISK_LIMITS.RTH_END_HOUR * 3600 + RISK_LIMITS.RTH_END_MINUTE * 60;

  // Subtract buffer from end time to prevent executing at 3:59:59
  const effectiveEndSeconds = endSeconds - RISK_LIMITS.RTH_END_BUFFER_SECONDS;

  const withinRTH = currentSeconds >= startSeconds && currentSeconds < effectiveEndSeconds;

  const timeStr = `${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}:${second.toString().padStart(2, '0')}`;
  console.log(`[RiskManager] Current ET time: ${timeStr}, Within RTH: ${withinRTH}`);

  if (!withinRTH && currentSeconds >= effectiveEndSeconds && currentSeconds < endSeconds) {
    console.log(`[RiskManager] Within 30-second buffer before market close - blocking trades`);
  }
...
function canExecuteTrade(webhookId = null) {
  ...
  if (!isWithinRTH()) {
    ...
    const endMinutes = RISK_LIMITS.RTH_END_HOUR * 60;

    if (currentMinutes >= endMinutes - 1 && currentMinutes < endMinutes) {
      return {
        allowed: false,
        reason: 'Too close to market close (within 30 seconds)',
      };
    }
