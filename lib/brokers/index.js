...
function createProjectXClient(accountConfig, brokerName, brokerType) {
  ...
  return {
    ...
    async placeBracketOrder(action, stopPrice, tpPrice, quantity = 1, options = {}) {
      const { skipCleanup = false, tpQuantity = quantity } = options;
      const runnerQuantity = quantity - tpQuantity;

      console.log(`[${brokerName}] Placing bracket order: ${action.toUpperCase()}, Stop: ${stopPrice}, TP: ${tpPrice}`);
      console.log(`[${brokerName}] Quantities - Entry: ${quantity}, SL: ${quantity}, TP: ${tpQuantity}${runnerQuantity > 0 ? `, Runner: ${runnerQuantity}` : ''}`);
      console.log(`[${brokerName}] skipCleanup: ${skipCleanup}`);

      // Cleanup existing positions and orders (unless already handled by caller)
      if (!skipCleanup) {
        try {
          ...
          console.error(`[${brokerName}] Cleanup failed:`, cleanupError.message);
          // Continue anyway - try to place the entry
          console.log(`[${brokerName}] Proceeding with entry despite cleanup failure`);
        }
      } else {
        console.log(`[${brokerName}] Skipping cleanup (already handled by caller)`);
        // Still cancel orphaned orders even if we skip position cleanup
        try {
          const openOrders = await getOpenOrders();
          if (openOrders.length > 0) {
            console.log(`[${brokerName}] Cancelling ${openOrders.length} orphaned order(s)...`);
            for (const order of openOrders) {
              await cancelOrder(order.id || order.orderId);
            }
          }
        } catch (orderError) {
          console.warn(`[${brokerName}] Could not cancel orphaned orders:`, orderError.message);
        }
        // Wait for orders to cancel
        await new Promise(resolve => setTimeout(resolve, 300));
      }

      // STEP 2: Close existing positions (for reversals) - OPTIONAL, fail gracefully
      console.log(`[${brokerName}] Step 2: Checking for positions to close...`);
      try {
        const positions = await getPositions();
        console.log(`[${brokerName}] Found ${positions.length} position(s):`, JSON.stringify(positions));
        if (positions.length > 0) {
          console.log(`[${brokerName}] Closing ${positions.length} existing position(s) for reversal...`);
          const closeResult = await this.closeAllPositions();
          console.log(`[${brokerName}] Close result:`, JSON.stringify(closeResult));
          // Wait for positions to settle
          await new Promise(resolve => setTimeout(resolve, 500));
        }
      } catch (posError) {
        console.warn(`[${brokerName}] Could not fetch positions (API may not support it): ${posError.message}`);
        console.log(`[${brokerName}] Proceeding without position check - bracket order will handle cleanup`);
      }

      // STEP 3: Place market entry
      console.log(`[${brokerName}] Step 3: Placing market ${action.toUpperCase()} entry...`);
      ...
      console.log(`[${brokerName}] Step 4: Placing stop loss at ${stopPrice}...`);
      const stopSide = action.toLowerCase() === 'buy' ? 'sell' : 'buy';
      let stopOrder;
      try {
        stopOrder = await placeStopOrder(stopSide, stopPrice, quantity);
        console.log(`[${brokerName}] Stop loss placed:`, JSON.stringify(stopOrder));
      } catch (e) {
        console.error(`[${brokerName}] CRITICAL: Stop loss failed!`, e.message);
        throw new Error(`Stop loss failed after entry. UNPROTECTED POSITION! Error: ${e.message}`);
      }

      // STEP 5: Place take profit (may be partial quantity for runner strategy)
      console.log(`[${brokerName}] Step 5: Placing take profit at ${tpPrice} for ${tpQuantity} contract(s)...`);
      let tpOrder;
      try {
        tpOrder = await placeLimitOrder(stopSide, tpPrice, tpQuantity);
        console.log(`[${brokerName}] Take profit placed:`, JSON.stringify(tpOrder));
        if (runnerQuantity > 0) {
          console.log(`[${brokerName}] Runner strategy active: ${runnerQuantity} contract(s) will run until next signal`);
        }
      } catch (e) {
        console.warn(`[${brokerName}] Take profit order failed, position protected by stop loss only:`, e.message);
      ...
      return { success: true, entry: entryOrder, stopLoss: stopOrder, takeProfit: tpOrder, runnerQuantity };
    },
