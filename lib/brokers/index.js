...
function createProjectXClient(accountConfig, brokerName, brokerType) {
  ...
  async function getPositions() {
    ...
    const response = await retryWithBackoff(async () => {
      return await fetchWithTimeout(`${BASE_URL}/Position/search`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  async function getOpenOrders() {
    ...
    const response = await retryWithBackoff(async () => {
      return await fetchWithTimeout(`${BASE_URL}/Order/search`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ accountId }),
      });
  ...
  return {
    ...
    async placeBracketOrder(action, stopPrice, tpPrice, quantity = 1) {
      console.log(`[${brokerName}] === BRACKET ORDER START ===`);
      console.log(`[${brokerName}] Action: ${action.toUpperCase()}, Stop: ${stopPrice}, TP: ${tpPrice}`);

      // STEP 1: Cancel all open orders FIRST (SL/TP from previous position)
      console.log(`[${brokerName}] Step 1: Checking for open orders to cancel...`);
      const openOrders = await getOpenOrders();
      console.log(`[${brokerName}] Found ${openOrders.length} open order(s)`);
      if (openOrders.length > 0) {
        for (const order of openOrders) {
          const orderId = order.id || order.orderId;
          console.log(`[${brokerName}] Cancelling order ${orderId}:`, JSON.stringify(order));
          try {
            await cancelOrder(orderId);
          } catch (cancelErr) {
            console.warn(`[${brokerName}] Failed to cancel order ${orderId}:`, cancelErr.message);
          }
        }
